...
<body>
	...
	<div class='do-chat'>
		...
		<input type="hidden" class="to_user_id" >
	</div>
	...
	layui.use(['element'], function(){
		...
		element.on('tabDelete(chat-win)', function(data){
		  ...
		  if($('.chat .layui-tab-title li').size()==0){
		  	window.parent.chat_layer.close(window.parent.chat_layer.index); 
		  	window.parent.chat_iframe_win = null;
		  	window.parent.chat_body = null;
		  	window.parent.chat_user_ids = [];
		  }
		});
	...
	/*获取当前正在对话的用户*/
	function get_chat_user_id(){
		return $('.to_user_id').val();
	}
	/*显示对方聊天内容*/
	function add_user_chat_content_to(data){
		data.content = data.content.replace(/\n/g,"<br>");
		$('.layui-show').html(function(k,v){
			var _content = `<div class='chat-user-left'>
					<div class="layui-anim layui-anim-fadein user-img">
						<img  src="image/`+data.from_user_face+`.png" alt="" >
						<ul>
							<li class='user-title cut-text'>`+data.add_time+` `+data.from_user_name+`</li>
	...
	/*显示我方聊天内容*/
	function add_user_chat_content_from(data){
		data.content = data.content.replace(/\n/g,"<br>");
		$('.layui-show').html(function(k,v){
	...
	/*获取未读的内容*/
	function get_unread_content(list){
		for (var i = 0; i < list.length; i++) {
			var data = list[i];
			add_user_chat_content_to(data);
		};
	}
	...
	$(function(){
		$('.do-send').click(function(){
			...
			window.parent.chat_friend(to_user_id,send_content);
			$('.send_content').val('');
		});
	});
